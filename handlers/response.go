...
func SuccessArrRespond(fields []*models.Trivia, writer http.ResponseWriter) {
	// var fields["status"] := "success"
	_, err := json.Marshal(fields)
	type data struct {
		People     []*models.Trivia `json:"data"`
		Statuscode int              `json:"status"`
		Message    string           `json:"msg"`
	}
...
func SuccessRespond(fields models.Trivia, writer http.ResponseWriter) {
	_, err := json.Marshal(fields)
	type data struct {
		Trivia     models.Trivia `json:"data"`
		Statuscode int           `json:"status"`
		Message    string        `json:"msg"`
	}
	temp := &data{Trivia: fields, Statuscode: 200, Message: "success"}
	if err != nil {
		ServerErrResponse(err.Error(), writer)
	}
